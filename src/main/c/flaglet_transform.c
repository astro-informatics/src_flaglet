...
int flaglet_f_size(const flaglet_parameters_t *parameters)
{
	int L = parameters->L;
	int P = parameters->P;
	return flag_core_f_size_mw(L, P);
}
...
/*!
 * Perform adjoint multiresolution wavelet transform in FLAG space (from precomputed kernels, gives FLAG coefficients).
 * 3D spherical wavelets : analysis in FLAG-harmonic space.
 */
	...
	for (p = 0; p < P; p++){
		for (l = ABS(parameters->spin); l < L; l++){
			for (m = -l; m <= l ; m++){
				ssht_sampling_elm2ind(&lm_ind, l, m);
				indlmp = p * L * L  +  lm_ind;
				flmp[indlmp] += f_scal_lmp[indlmp] * sqrt((4.0*PI)/(2.0*l+1.0)) * conj(scal_lmp[p*L+l]) ;
			}
		}
	}
}
 ...
 * 3D spherical wavelets : synthesis in FLAG-harmonic space.
 */
void flaglet_synthesis_lmnp_adjoint(complex double *f_wav_lmnp, complex double *f_scal_lmp, const complex double *flmp, const complex double *wav_lmp, const double *scal_lmp, const flaglet_parameters_t *parameters)
{
	int offset, jl, jp, l, m, n, p, lmn_size, lm_ind, ln_ind, indlmn, indjjlnp, indlmp;
	complex double psi;
	int L = parameters->L;
	...
	for (p = 0; p < P; p++){
		for (l = ABS(parameters->spin); l < L; l++){
			for (m = -l; m <= l ; m++){
				ssht_sampling_elm2ind(&lm_ind, l, m);
				indlmp = p * L * L  +  lm_ind;
				f_scal_lmp[indlmp] = flmp[indlmp] * sqrt((4.0*PI)/(2.0*l+1.0)) * conj(scal_lmp[p*L+l]);
			}
		}
	}
...
/*!
 * Perform adjoint wavelet transform in real space (from scratch, gives pixel space components).
 * Sampling scheme : MW sampling.
 * 3D spherical wavelets : analysis adjoint in real space.
 */
void flaglet_analysis_adjoint(complex double *f, const complex double *f_wav, const complex double *f_scal, const flaglet_parameters_t *parameters)
{
	...
	complex double *flmp;
	flag_core_allocate_flmn(&flmp, L, P);
	flaglet_analysis_lmnp_adjoint(flmp, f_wav_lmnp, f_scal_lmp, wav_lmp, scal_lmp, parameters);
 ...
 * Perform adjoint wavelet transform in real space (from scratch, gives pixel space components).
 * Sampling scheme : MW sampling.
 * 3D spherical wavelets : adjoint synthesis in real space.
 */
void flaglet_synthesis_adjoint(complex double *f_wav, complex double *f_scal, const complex double *f, const flaglet_parameters_t *parameters)
{
	double tau = parameters->tau;
	int L = parameters->L;
	int P = parameters->P;
	...
	complex double *flmp, *f_wav_p;
	flag_core_allocate_flmn(&flmp, L, P);
	flag_adjoint_synthesis(flmp, f, nodes, P, L, tau, P, spin);
	...
	complex double *f_wav_lmnp, *f_scal_lmp;
	flaglet_allocate_f_wav_lmnp(&f_wav_lmnp, &f_scal_lmp, parameters);
	flaglet_synthesis_lmnp_adjoint(f_wav_lmnp, f_scal_lmp, flmp, wav_lmp, scal_lmp, parameters);

	int offset_lmnp, offset, jl, jp, lmn_size, f_size, p;
	int Nj = MIN(N,L);

	flag_adjoint_analysis(f_scal, f_scal_lmp, L, tau, P, spin);
	...
	for (jp = parameters->J_min_p; jp <= J_p; jp++){
		...
		for (jl = parameters->J_min_l; jl <= J_l; jl++){
	        ...
	        flag_spherlaguerre_mapped_analysis_adjoint(f_wav + offset, f_wav_p, nodes, weights, tau, bandlimit_p, f_size);
			offset += f_size * bandlimit_p;
	        free(f_wav_p);
		}
	}
