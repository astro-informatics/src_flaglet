...
/**
 * MATLAB interface: b3let_axisym_tilling_mex.
 * This function for internal use only.
 * Compute tilling in l-n harmonic space for axisymmetric wavelets.
 *
 * Usage: 
 *   [kappa kappa0] = b3let_axisym_tilling_mex(B_l, B_n, L, N, J_min_l, J_min_n);
 *
 */
                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  if(nrhs!=6) {
    mexErrMsgIdAndTxt("b3let_axisym_tilling_mex:InvalidInput:nrhs",
		      "Require six inputs.");
  }
  if(nlhs!=2) {
    mexErrMsgIdAndTxt("b3let_axisym_tilling_mex:InvalidOutput:nlhs",
		      "Require two outputs.");
  }
  ...
  if( !mxIsDouble(prhs[iin]) || 
    ...
    mexErrMsgIdAndTxt("b3let_axisym_tilling_mex:InvalidInput:waveletParameter",
          "Wavelet parameter B_l must be integer.");
  }
  B_l = (int)mxGetScalar(prhs[iin]);

  if (mxGetScalar(prhs[iin]) > (double)B_l || B_l <= 1)
    mexErrMsgIdAndTxt("b3let_axisym_tilling_mex:InvalidInput:bandLimitNonInt",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("b3let_axisym_tilling_mex:InvalidInput:LbandLimit",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("b3let_axisym_tilling_mex:InvalidInput:J_min_l",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("b3let_axisym_tilling_mex:InvalidInput:J_min_n",
  ...
  // Run S2LET function
  b3let_axisym_tilling(kappa, kappa0, B_l, B_n, L, N, J_min_l, J_min_n);
