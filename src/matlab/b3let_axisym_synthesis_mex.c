...
/**
 * MATLAB interface: b3let_axisym_synthesis.
 ...
 * Usage: 
 *   f = ...
 *        b3let_axisym_synthesis_mex(f_wav, f_scal, B_l, B_n, L, N, J_min_l, J_min_n, reality));
 *
 */
void mexFunction( int nlhs, mxArray *plhs[],
                  int nrhs, const mxArray *prhs[])
{
  int n, p, t, i, j, f_m, f_n, reality;
  ...
  if( !mxIsDouble(prhs[iin]) || 
    ...
    mexErrMsgIdAndTxt("b3let_axisym_synthesis_mex:InvalidInput:waveletParameter",
          "Wavelet parameter B_l must be integer.");
  }
  B_l = (int)mxGetScalar(prhs[iin]);

  if (mxGetScalar(prhs[iin]) > (double)B_l || B_l <= 1)
    mexErrMsgIdAndTxt("b3let_axisym_synthesis_mex:InvalidInput:bandLimitNonInt",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("b3let_axisym_synthesis_mex:InvalidInput:LbandLimit",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("b3let_axisym_synthesis_mex:InvalidInput:J_min_l",
  ...
  if( !mxIsDouble(prhs[iin]) || 
      mxIsComplex(prhs[iin]) || 
      mxGetNumberOfElements(prhs[iin])!=1 ) {
    mexErrMsgIdAndTxt("b3let_axisym_synthesis_mex:InvalidInput:J_min_n",
  ...
  if(reality){
    f_r = (double*)calloc( L * (2*L-1) * (N+1), sizeof(double));
    b3let_axisym_wav_synthesis_real(f_r, f_wav_r, f_scal_r, R, B_l, B_n, L, N, J_min_l, J_min_n);
  }else{
    f = (complex double*)calloc( L * (2*L-1) * (N+1), sizeof(complex double));
  ...
  if(reality){
    ...
    for(n=0; n<N+1; n++) 
      for(i=0; i<ntheta*nphi; i++) 
        f_real[i*(N+1) + n] = creal(f_r[n*ntheta*nphi + i]);
    ...
    for(n=0; n<N+1; n++) {    
      for(i=0; i<ntheta*nphi; i++) {
        f_real[i*(N+1) + n] = creal( f[n*ntheta*nphi + i] );
        f_imag[i*(N+1) + n] = cimag( f[n*ntheta*nphi + i] );
      }
    }
  }
